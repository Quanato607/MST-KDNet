...
from models import build_MSTKDNet, build_MSTKDNet_wo_gsm
from models.discriminator import get_style_discriminator
from solver import make_optimizer_double
from losses import get_losses_divide, bce_loss, Dice, get_current_consistency_weight
import os
import torch
...
## Main section
task_name = 'fets_mstkdnet_wo_gsm'
...
model_full, model_missing = build_MSTKDNet_wo_gsm(inp_dim1 = 4, inp_dim2 = 4)
...
if os.path.exists(latest_model_path):
    model_full, model_missing, d_style, optimizer, epoch, best_dice = load_old_model(model_full, model_missing, d_style, optimizer, latest_model_path)
else:
    best_dice = 0.0
        ...
        for phase in ['train', 'eval']:
            ...
            for batch_id, (batch_x, batch_y) in tqdm(enumerate(loader), total=total, desc="Training Batches"):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    loss_dict['loss_Co'] += (unnetr_loss * (0.2) + evd_loss * (1e8) + slkd_loss)
        ...
        for phase in ['eval']:
            ...
            for batch_id, (batch_x, batch_y) in tqdm(enumerate(loader), total=len(loader), desc="test Batches"):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    mask_tensor = mask_tensor.cuda(non_blocking=True)
                    batch_xn = batch_x * mask_tensor
                    seg_m, _, _, _, _, _, _ = model_missing(batch_xn[:, 0:])
